...
# while cutting engine time on large photos by several×.
OCR_MAX_DIM = 1600


def preprocess_image_for_ocr(image: np.ndarray) -> np.ndarray:
    """Enhance image for OCR: downscale, grayscale, denoise, CLAHE."""
    # Bounded downscale: keep aspect, cap the long edge at OCR_MAX_DIM.
    h, w = image.shape[:2]
    long_edge = max(h, w)
    if long_edge > OCR_MAX_DIM:
        scale = OCR_MAX_DIM / float(long_edge)
        image = cv2.resize(image, (max(1, round(int(w * scale))), max(1, round(int(h * scale)))), interpolation=cv2.INTER_AREA)
