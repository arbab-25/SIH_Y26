...
import { ShieldCheck, AlertTriangle, AlertCircle, FileText, Download, Send, ChevronRight, Check } from 'lucide-react';
...
export const ScanSummary: React.FC<ScanSummaryProps> = ({
  ...
  // Pie chart slices from scan result or fallback
  const pieData = scan.confidence_pie?.length > 0
    ? scan.confidence_pie
    : [
        { name: 'High (≥90%)', value: 8, color: '#16A34A', fields: ['Net Quantity', 'MRP', 'Brand'] },
        { name: 'Medium (75–89%)', value: 3, color: '#0E7490', fields: ['Manufacturer', 'Consumer Care'] },
        { name: 'Low (<75%)', value: 1, color: '#D97706', fields: ['Date of Packing'] },
      ];
          ...
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-xs text-slate-500 font-semibold">
                Processed in {scan.processing_time_ms || 420}ms
              </span>
            </div>
            <h2 className="text-xl font-bold text-[#12355B] mt-1.5">
              {scan.product?.name || 'Pre-packaged Commodity'}
            </h2>
            <p className="text-xs text-slate-600">
              {scan.product?.manufacturer_name || 'Manufacturer details verified'}
            </p>
          </div>
        </div>
          ...
          <div className="pt-3 border-t border-slate-100 space-y-2">
            <div className="flex items-center justify-between text-xs font-bold text-slate-700">
              ...
              <span className="text-base text-[#0E7490]">
                {(scan.avg_ocr_confidence || 88.5).toFixed(1)}%
              </span>
            </div>
